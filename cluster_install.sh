...
EOF
    ...
    ssh ${node[0]} "sudo curl -fsSL https://pkgs.k8s.io/core:/stable:/v1.29/deb/Release.key | sudo gpg --dearmor -o  /etc/apt/keyrings/kubernetes-archive-keyring.gpg"

    ssh ${node[0]} "echo 'deb [signed-by=/etc/apt/keyrings/kubernetes-archive-keyring.gpg] https://pkgs.k8s.io/core:/stable:/v1.29/deb/ /' | sudo tee /etc/apt/sources.list.d/kubernetes.list"
...
worker_install () {
    ...
    ssh ${node[0]} "sudo nft 'add chain inet kubernetes input { type filter hook input priority 0 ; }'"
    ssh ${node[0]} "sudo nft add rule inet kubernetes input tcp dport 10250 accept"
    ssh ${node[0]} "sudo nft add rule inet kubernetes input tcp dport 30000-32767 accept"
    ssh ${node[0]} "echo flush ruleset | sudo tee /etc/nftables.conf"
    ssh ${node[0]} "sudo nft list ruleset | sudo tee -a /etc/nftables.conf"
